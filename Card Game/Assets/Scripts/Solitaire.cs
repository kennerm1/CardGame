...
public class Solitaire : MonoBehaviour
{
    ...
    {
        randRange = Random.Range(0, 51);
        int n = list.Count;
        while(n > 1)
        {
            int k = randRange.Next(n);
            n--;
            T temp = list[k];
            list[k] = list[n];
            list[n] = temp;
    ...
    void SolitaireDeal() // physically moves the cards into their starting piles, and flips the top card in each pile
    {
        for (int i = 0; i < 7; i++)
        {
            ...
            {
                GameObject newCard = Instantiate(cardPrefab, new Vector3(bottmPos[i].transform.position.x, bottomPos[i].transform.position.y - yOffset, bottomPos[i].transform.position.z - zOffset), Quaternion.identity, bottomPos[i].transform);
                newCard.name = card; //names the card so it can be referenced later
                if (card == bottoms[i].get(bottoms[i].Count - 1))
                {
    ...
    void SolitaireSort() //sorts all the starting cards into their individual piles
    {
        ...
        for (int i = 1; i < 8; i++) //for each pile
        {
            ...
            {
                bottoms.get(i-1).add(deck.get(cardCount));
    ...
    {
        GameObject drawnCard = GameObject.Find(deck.get(0)); // find the revealed card gameobject
        drawnCard.transform = new Vector3(drawPilePos.transform.position.x, drawPilePos.transform.position.y, drawPilePos.transform.position.z); // move the current revealed card
        drawnCard.getComponent<Selectable>().faceUp = false; // flip it back over
        deck.Insert(deck.Count-1, deck.get(0)); // add a copy of the revealed card to the back
        deck.RemoveAt(0); // remove the revealed card
        drawnCard = GameObject.Find(deck.get(0)); // set the revealed card to the next card
        drawnCard.transform = new Vector3(drawnPos.transform.position.x, drawnPos.transform.position.y, drawnPos.transform.position.z); // move the new first card to the right pos
